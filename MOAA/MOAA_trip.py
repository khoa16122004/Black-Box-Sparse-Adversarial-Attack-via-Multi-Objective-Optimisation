...
class PopulationTrip:
    def __init__(self, solutions: list, loss_function, include_dist, objective2_fn=None, objective3_fn=None):
        self.population = solutions
        self.fronts = None
        self.loss_function = loss_function
        self.include_dist = include_dist
        self.objective2_fn = objective2_fn
        self.objective3_fn = objective3_fn

    def evaluate_trip(self):
        ...
        if self.objective3_fn is not None:
            if hasattr(self.objective3_fn, "batch"):
                obj3_batch = self.objective3_fn.batch(imgs_adv)
            else:
                obj3_batch = [self.objective3_fn(img) for img in imgs_adv]
        else:
            obj3_batch = [0.0 for _ in self.population]

        for idx, pi in enumerate(self.population):
            fs = fs_batch[idx]
            pi.is_adversarial = bool(fs[0])
            pi.fitnesses = np.array([float(fs[1]), float(obj2_batch[idx]), float(obj3_batch[idx])], dtype=float)
            pi.loss = float(fs[1])
...
class AttackTrip:
    ...
    def completion_procedure(self, population: PopulationTrip, loss_function, fe, success):
        adversarial_labels = []
    ...
    def attack(self, loss_function):
        ...
        objective3_fn = self.params.get("objective3_fn", None)
        population = PopulationTrip(init_solutions, loss_function, self.params["include_dist"], objective2_fn, objective3_fn)
        population.evaluate_trip()
        ...
        if self.verbose:
            print(
                f"[MOAA-Trip] start | pop_size={self.params['pop_size']} | iterations={self.params['iterations']} "
        ...
        for it in tqdm(range(1, self.params["iterations"])):
            ...
            if self.verbose and (it == 1 or it % max(self.print_every, 1) == 0):
                ...
                min_obj2 = float(obj_mins[2]) if len(obj_mins) > 2 else float("nan")
                print(
                    f"[MOAA-Trip] iter={it}/{self.params['iterations'] - 1} | queries={fe} "
                    f"| min_obj0={min_obj0:.6f} | min_obj1={min_obj1:.6f} | min_obj2={min_obj2:.6f} "
            ...
            if len(adv_solns) > 0:
                ...
                if self.verbose:
                    print(
                        f"[MOAA-Trip] success | iter={it} | queries={fe} "
            ...
            offsprings = PopulationTrip(children, loss_function, self.params["include_dist"], objective2_fn, objective3_fn)
            fe += len(offsprings.population)
            offsprings.evaluate_trip()
            ...
            population = PopulationTrip(new_solutions, loss_function, self.params["include_dist"], objective2_fn, objective3_fn)
        population.fronts = fast_nondominated_sort(population.population)
        self.fitness.append(self.objective_mins(population.population))
        self.completion_procedure(population, loss_function, fe, False)
        if self.verbose:
            print(f"[MOAA-Trip] finished | success=False | queries={fe} | elapsed={time.time() - start:.2f}s")
        return
