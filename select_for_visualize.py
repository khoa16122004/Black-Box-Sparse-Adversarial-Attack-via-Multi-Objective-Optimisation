...
def process_all_samples(result_dir):
    ...
    for class_name in os.listdir(result_dir):
        ...
        best_rank0_advmap = os.path.join(sample_path, "rank0", f"map_{int(best_rank0_id):03d}.png") # adv map


        # calculate spearman rank correlation
        saliency_map = np.array(Image.open(original_map).convert("L"))
        perturbation_map = np.array(Image.open(best_rank0_advmap).convert("L"))
        spearman_corr = spear_rank_correlation_cal(saliency_map, perturbation_map)
        best["spearman_corr"] = spearman_corr
        # print(spearman_corr)
    ...
    mean_spearman = float(np.mean([b["spearman_corr"] for b in best_candidates])) if best_candidates else 0.0
